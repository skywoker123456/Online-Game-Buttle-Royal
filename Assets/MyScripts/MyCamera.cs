...
namespace CoverShooter
{
	...
	public class MyCamera : CharacterCamera
	{
		/// <summary>
		...
		public float[] _zoomMultipliers = new float[] { 1, 1.5f, 2, 2.5f };
		...
		public float collisionCushion = 2f;
		...
		void Start()
		{
			...
			transform.position = Target.transform.position + Vector3.up * CameraHeight;
		...
		void CameraCollisions()
		{
			...
			RaycastHit camRayHit;
			camRay.origin = _camera.transform.position;
			//camRay.direction = -tilt.up;
			camRay.direction = Target.transform.position - _camera.transform.position;

			if (Physics.Raycast (camRay, out camRayHit, camDistance, collisionMask)) {

				if (camRayHit.collider.gameObject.tag == "Terrain") {

					Vector3 CalmOffset = new Vector3 (-5, 9, 0);
					Debug.DrawLine(camRay.origin, camRay.origin + camRay.direction * camDistance, Color.blue);
				} 
	...
	//		}
		...
		//	}
			...
			if (collisionDebug)
				Debug.DrawLine(camRay.origin, camRay.origin + camRay.direction * camDistance, Color.red);
		}
